<?php
/**
 * Extend the Theme_Upgrader class.
 *
 ...
 *
 */
class Mystery_Theme_Updater extends Theme_Upgrader {
	public function install_package( $args = array() ) {
		...
		if ( is_dir( $destination ) && file_exists( "$destination/style.css" ) ) {
			...
			$this->strings['installing_package'] = __( 'Upgrading the theme&#8230;', 'mystery-theme-updater' );
			$this->strings['remove_old'] = __( 'Backing up the old version of the theme&#8230;', 'mystery-theme-updater' );
		}
	...
	public function clear_destination( $destination ) {
		...
		if ( ! is_wp_error( $backup_url ) ) {
			...
			$this->skin->feedback( sprintf( __( 'A backup zip file of the old theme version can be downloaded <a href="%1$s">here</a>.', 'mystery-theme-updater' ), $backup_url ) );
		...
		$this->skin->feedback( __( 'Moving the old version of the theme to a new directory&#8230;', 'mystery-theme-updater' ) );
		...
		if ( is_dir( "$directory/$new_name" ) ) {
			...
			$this->skin->error( __( 'Unable to find a new directory name to move the old version of the theme to. No backup will be created.', 'mystery-theme-updater' ) );
		} else {
			$result = $wp_filesystem->move( $destination, "$directory/$new_name" );

			if ( $result ) {
				/* translators: 1: new theme directory name */
				$this->skin->feedback( sprintf( __( 'Moved the old version of the theme to a new theme directory named %1$s. This directory should be backed up and removed from the site.', 'mystery-theme-updater' ), "<code>$new_name</code>" ) );
			} else {
				$this->skin->error( __( 'Unable to move the old version of the theme to a new directory. No backup will be created.', 'mystery-theme-updater' ) );
			}
		}
	...
	private function create_backup( $directory ) {
		...
		if ( ! is_dir( $zip_path ) ) {
			return new WP_Error( 'mtu-cannot-backup-no-destination-path', __( 'A theme backup can not be created since a destination path for the backup file could not be found.', 'mystery-theme-updater' ) );
		}
		...
		if ( 0 === $zip_result ) {
			...
			return new WP_Error( 'mtu-cannot-backup-zip-failed', sprintf( __( 'A theme backup can not be created as creation of the zip file failed with the following error: %1$s', 'mystery-theme-updater' ), $archive->errorInfo( true ) ) );
		}
		...
		$attachment = array(
			...
			'post_title'     => sprintf( __( 'Theme Backup - %1$s - %2$s', 'mystery-theme-updater' ), $data['name'], $data['version'] ),
			'post_content'   => '',
		);
